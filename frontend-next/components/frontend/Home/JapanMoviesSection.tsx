...
function JapanMoviesSection() {
    const [dataJapanMovies, setDataJapanMovies] = useState<Movie[]>([]);
    ...
    useEffect(() => {
        const fetchDatas = async () => {
            setLoading(true);
            try {
                const data = await movieService.dataJapanMovies();
                setDataJapanMovies(data);
            } catch (e) {
                console.error("Fetch Japan movies error", e);
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="w-full pt-4 pb-6 px-3 lg:px-5 xl:px-6 overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-5">
                <div className="flex items-center gap-2 sm:gap-3">
                    <div className="w-1 h-5 sm:h-7 rounded-full bg-gradient-to-b from-rose-400 to-red-600 shrink-0" />
                    <div>
                        ...
                        <p className="hidden sm:block text-[11px] text-gray-500 mt-0.5">
                            Anime, drama, điện ảnh Nhật Bản mới nhất
                        </p>
                    </div>
                </div>

                <Link href="/loc-phim?country=nhat-ban"
                                ...
                                <Link href={`/phim/${movie.slug}`}>
                                    <div className="group cursor-pointer w-full">
                                        ...
                                        <div className="movie-card relative w-full rounded-xl overflow-hidden" style={{ aspectRatio: "2/3" }}>
                                            ...
                                            <div className="absolute bottom-0 inset-x-0 p-1.5 sm:p-2.5">
                                                <div className="flex items-center gap-1">
                                                    {movie.quality && (
                                                        <span className="flex-1 py-0.5 px-0.5 text-[7px] sm:text-[9px] font-bold rounded-md bg-rose-500/25 border border-rose-400/30 text-rose-300 text-center backdrop-blur-sm truncate">
                                                            {movie.quality}
                                                        </span>
                                                    )}
                                                    {movie.time && (
                                        ...
                                        <div className="mt-1.5 sm:mt-2.5 px-0.5 text-center">
                                            <h3
                                                className="text-[10px] sm:text-[12px] lg:text-[13px] text-white font-semibold truncate group-hover:text-rose-300 transition-colors duration-300 leading-snug"
...
export default memo(JapanMoviesSection);
