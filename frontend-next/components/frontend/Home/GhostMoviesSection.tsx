...
function GhostMoviesSection() {
    const [dataGhostMovies, setDataGhostMovies] = useState<Movie[]>([]);
    ...
    useEffect(() => {
        const fetchDatas = async () => {
            setLoading(true);
            try {
                const data = await movieService.dataGhostMovies();
                setDataGhostMovies(data);
            } catch (e) {
                console.error("Fetch ghost movies error", e);
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="w-full pt-4 pb-6 px-3 lg:px-5 xl:px-6 overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-5">
                <div className="flex items-center gap-2 sm:gap-3">
                    {/* Accent tím ma quái — horror */}
                    <div className="w-1 h-5 sm:h-7 rounded-full bg-gradient-to-b from-violet-400 to-purple-800 shrink-0" />
                    <div>
                        ...
                        <p className="hidden sm:block text-[11px] text-gray-500 mt-0.5">
                            Phim kinh dị rùng rợn nhất — dám xem không?
                        </p>
                    </div>
                </div>

                <Link href="/loc-phim?category=kinh-di"
                                ...
                                <Link href={`/phim/${movie.slug}`}>
                                    <div className="group cursor-pointer w-full">
                                        ...
                                        <div className="movie-card relative w-full rounded-xl overflow-hidden" style={{ aspectRatio: "2/3" }}>
                                            ...
                                            {/* Gradient tối hơn — phim ma cần tối */}
                                            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent" />

                                            {/* Play hover */}
                                            <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                                                <div className="w-10 h-10 rounded-full bg-violet-500/20 backdrop-blur-sm border border-violet-400/40 flex items-center justify-center scale-75 group-hover:scale-100 transition-transform duration-300">
                                            ...
                                            <div className="absolute bottom-0 inset-x-0 p-1.5 sm:p-2.5">
                                                <div className="flex items-center gap-1">
                                                    {movie.quality && (
                                                        <span className="flex-1 py-0.5 px-0.5 text-[7px] sm:text-[9px] font-bold rounded-md bg-violet-500/25 border border-violet-400/30 text-violet-300 text-center backdrop-blur-sm truncate">
                                                            {movie.quality}
                                                        </span>
                                                    )}
                                                    {movie.time && (
                                        ...
                                        <div className="mt-1.5 sm:mt-2.5 px-0.5 text-center">
                                            <h3
                                                className="text-[10px] sm:text-[12px] lg:text-[13px] text-white font-semibold truncate group-hover:text-violet-300 transition-colors duration-300 leading-snug"
            ...
            <style>{`
                @media (max-width: 639px) {
                    ...
                    .ghost-swiper .swiper-button-prev { display: none !important; }
                }
                .ghost-swiper .swiper-button-next,
                ...
                .ghost-swiper .swiper-button-next:hover,
                .ghost-swiper .swiper-button-prev:hover {
                    background: rgba(167,139,250,0.15); border-color: rgba(167,139,250,0.4);
                }
                .ghost-swiper .swiper-button-next::after,
                .ghost-swiper .swiper-button-prev::after { font-size: 11px; font-weight: 900; color: white; }
                .ghost-swiper .swiper-button-disabled { opacity: 0 !important; }
                .ghost-swiper { overflow: hidden !important; }
                .ghost-swiper .swiper-wrapper { overflow: visible; }
            `}</style>
        </div>
    );
}
export default memo(GhostMoviesSection);
