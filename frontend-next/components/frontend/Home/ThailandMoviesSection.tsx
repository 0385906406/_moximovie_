...
function ThailandMoviesSection() {
    const [dataThailandMovies, setDataThailandMovies] = useState<Movie[]>([]);
    ...
    useEffect(() => {
        const fetchDatas = async () => {
            setLoading(true);
            try {
                const data = await movieService.dataThaiLandMovies();
                setDataThailandMovies(data);
            } catch (e) {
                console.error("Fetch Thailand movies error", e);
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="w-full pt-4 pb-6 px-3 lg:px-5 xl:px-6 overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-4 sm:mb-5">
                <div className="flex items-center gap-2 sm:gap-3">
                    {/* Accent teal — màu cờ Thái */}
                    <div className="w-1 h-5 sm:h-7 rounded-full bg-gradient-to-b from-teal-400 to-cyan-600 shrink-0" />
                    <div>
                        ...
                        <p className="hidden sm:block text-[11px] text-gray-500 mt-0.5">
                            Drama Thái Lan mới cập nhật mỗi ngày
                        </p>
                    </div>
                </div>

                <Link href="/loc-phim?country=thai-lan"
                                ...
                                <Link href={`/phim/${movie.slug}`}>
                                    <div className="group cursor-pointer w-full">
                                        ...
                                        <div className="movie-card relative w-full rounded-xl overflow-hidden" style={{ aspectRatio: "2/3" }}>
                                            ...
                                            <div className="absolute bottom-0 inset-x-0 p-1.5 sm:p-2.5">
                                                <div className="flex items-center gap-1">
                                                    {movie.quality && (
                                                        <span className="flex-1 py-0.5 px-0.5 text-[7px] sm:text-[9px] font-bold rounded-md bg-teal-500/25 border border-teal-400/30 text-teal-300 text-center backdrop-blur-sm truncate">
                                                            {movie.quality}
                                                        </span>
                                                    )}
                                                    {movie.time && (
                                        ...
                                        <div className="mt-1.5 sm:mt-2.5 px-0.5 text-center">
                                            <h3
                                                className="text-[10px] sm:text-[12px] lg:text-[13px] text-white font-semibold truncate group-hover:text-teal-300 transition-colors duration-300 leading-snug"
            ...
            <style>{`
                @media (max-width: 639px) {
                    .thailand-swiper .swiper-button-next,
                    .thailand-swiper .swiper-button-prev { display: none !important; }
                }
                .thailand-swiper .swiper-button-next,
                .thailand-swiper .swiper-button-prev {
                    width: 32px; height: 32px; border-radius: 50%;
                    background: rgba(255,255,255,0.08); backdrop-filter: blur(8px);
                    border: 1px solid rgba(255,255,255,0.12);
                    transition: all 0.25s ease; top: 38%;
                }
                .thailand-swiper .swiper-button-next:hover,
                .thailand-swiper .swiper-button-prev:hover {
                    background: rgba(45,212,191,0.15); border-color: rgba(45,212,191,0.4);
                }
                .thailand-swiper .swiper-button-next::after,
                .thailand-swiper .swiper-button-prev::after { font-size: 11px; font-weight: 900; color: white; }
                .thailand-swiper .swiper-button-disabled { opacity: 0 !important; }
                .thailand-swiper { overflow: hidden !important; }
                .thailand-swiper .swiper-wrapper { overflow: visible; }
            `}</style>
        </div>
    );
}
export default memo(ThailandMoviesSection);
