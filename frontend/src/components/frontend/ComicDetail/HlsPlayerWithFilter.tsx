...
const HlsPlayerWithFilter: React.FC<HlsPlayerProps> = ({
    ...
    }, [originalPlaylist, filteredSegments]);

    /* ---------------------- Overlay nút play + UI Dahee ---------------------- */

    const handlePlayClick = () => {
        // mở web shop (giống component cũ)
    ...
    return (
        <div className="relative w-full h-full z-50">
            ...
            {/* 3 hình hiển thị bên trái */}
            {showLogo && (
                <div
                    className="
                        absolute lg:bottom-50 xl:bottom-100 -left-7 z-10
                        hidden sm:hidden lg:flex xl:flex flex-col items-stretch gap-2
                        max-w-[260px]
                    "
                    style={{
                        animation: "slideDisappearAppear 7s ease-in-out infinite"
                    }}
                >
                    {/* Fanpage */}
                    <div className="h-auto w-auto">
                        <img
                            src={`/${theme}/Image-Mini/icon-movie-detail.png`}
                            alt="Santa Claus and Mrs. Claus with Big Blank Signboard"
                            className="transform scale-x-[-1] h-[300px]"
                        />
                    </div>
                </div>
            )}
