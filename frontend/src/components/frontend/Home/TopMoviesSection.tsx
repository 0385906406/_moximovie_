...
function TopMoviesSection() {
    const [dataTopNewMovies, setDataTopNewMovies] = useState<Movie[]>([]);
    ...
    useEffect(() => {
        const fetchDatas = async () => {
            setLoading(true);
            try {
                const data = await movieService.dataTopNewMovies();
                setDataTopNewMovies(data);
            } catch (e) {
                console.error("Fetch top movies error", e);
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="w-full pt-4 pb-6 px-3 lg:px-5 xl:px-6 overflow-hidden">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                    ...
                    <div>
                        ...
                        <p className="hidden sm:block text-[11px] text-gray-500 mt-0.5">
                            Bảng xếp hạng phim lẻ được xem nhiều nhất
                        </p>
                    </div>
                </div>

                {/* Rank badge */}
                <div className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full border border-green-400/25 bg-green-400/8 text-green-400 text-[9px] sm:text-[11px] font-bold whitespace-nowrap">
                        ...
                        >
                            ...
                            <div className="group cursor-pointer w-full">
                                ...
                                <Link to={`/phim/${movie.slug}`}>
                                    <div
                                        className={`poster-mask relative w-full overflow-hidden rounded-xl ${i % 2 !== 0 ? "scale-x-[-1]" : ""}`}
                                ...
                                <div className="flex items-start gap-2 sm:gap-3 mt-2 sm:mt-3 px-0.5">
                                    {/* Số thứ tự gradient vàng — giữ nguyên */}
                                    <div
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <Link to={`/phim/${movie.slug}`}>
                                            <h3
                                                className="text-[11px] sm:text-sm text-white font-semibold truncate group-hover:text-green-300 transition-colors duration-300 leading-snug"
            ...
            <style>{`
                @media (max-width: 639px) {
                    .top-movies-swiper .swiper-button-next,
                    .top-movies-swiper .swiper-button-prev { display: none !important; }
                }
                .top-movies-swiper .swiper-button-next,
                .top-movies-swiper .swiper-button-prev {
                    width: 32px; height: 32px; border-radius: 50%;
                    background: rgba(255,255,255,0.08); backdrop-filter: blur(8px);
                    border: 1px solid rgba(255,255,255,0.12);
                    transition: all 0.25s ease; top: 35%;
                }
                .top-movies-swiper .swiper-button-next:hover,
                .top-movies-swiper .swiper-button-prev:hover {
                    background: rgba(74,222,128,0.15); border-color: rgba(74,222,128,0.4);
                }
                .top-movies-swiper .swiper-button-next::after,
                .top-movies-swiper .swiper-button-prev::after { font-size: 11px; font-weight: 900; color: white; }
                .top-movies-swiper .swiper-button-disabled { opacity: 0 !important; }
                .top-movies-swiper { overflow: hidden !important; }
                .top-movies-swiper .swiper-wrapper { overflow: visible; }
            `}</style>
        </div>
    );
}
export default memo(TopMoviesSection);
